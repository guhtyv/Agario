...
from menu import ConnectWindow #add
...
sock.connect((host, port)) #correct
...
eats = [Eat(randint(-2000, 2000), randint(-2000, 2000), 10,
      ...
      for _ in range(300)]
name_font = font.Font(None, 20) #add
while running:
  for e in event.get():
      if e.type == QUIT:
          running = False
...
#add
  ...
  if not lose:
      ...
      try:
          msg = f"{my_id},{my_player[0]},{my_player[1]},{my_player[2]},{name}" #correct
